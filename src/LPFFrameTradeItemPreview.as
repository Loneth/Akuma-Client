...
package {
    ...
    public class LPFFrameTradeItemPreview extends LPFFrame {
		...
		public var mcPreviewText:MovieClip;
		public var mcBG:MovieClip;
		public var mcDrag:SimpleButton;
		public var btnClose2:SimpleButton;
		public var dragonLeft:MovieClip;
		public var dragonRight:MovieClip;
		
        public function LPFFrameTradeItemPreview():void{
			drag = new Object();
            previewArgs = {};
			...
			mcDrag.addEventListener(MouseEvent.MOUSE_DOWN, onBankMoveClick, false, 0, true);
			btnClose2.addEventListener(MouseEvent.CLICK, onClose, false, 0, true);
		...
		private function onClose(_arg1:MouseEvent):void{
			...
			mcBG.visible = false;
			dragonLeft.visible = false;
			dragonRight.visible = false;
			...
			btnStorm.visible = false;
			btnLightning.visible = false;
			mcPreviewText.visible = false;
			clearPreview();
        }
		
		public function onBankMoveClick(_arg1:MouseEvent):void{
			drag.ox = this.x;
			drag.oy = this.y;
			drag.mox = stage.mouseX;
			drag.moy = stage.mouseY;
			stage.addEventListener(MouseEvent.MOUSE_UP, onBankMoveRelease, false, 0, true);
			this.addEventListener(Event.ENTER_FRAME, onBankMoveEnterFrame, false, 0, true);
        }

        public function onBankMoveRelease(_arg1:MouseEvent):void{
            stage.removeEventListener(MouseEvent.MOUSE_UP, onBankMoveRelease);
            this.removeEventListener(Event.ENTER_FRAME, onBankMoveEnterFrame);
        }

        public function onBankMoveEnterFrame(_arg1:Event){
        ...
        protected function fDraw():void{
            ...
            if (_local4 != null){
                ...
                TR.visible = false;
                mcDrag.visible = true;
				mcPreviewText.visible = true;
				mcBG.visible = true;
				dragonLeft.visible = true
				dragonRight.visible = true;
				...
				trace("ELEMENT: " + _local4.sElement);
				...
				if (isEquip){
                    tInfo.htmlText = "Please select an equipment to preview.";
				} else {
            ...
            btnDelete.visible = true;
            if (getLayout().sMode.toLowerCase().indexOf("shop") > -1 || getLayout().sMode.toLowerCase().indexOf("bank") > -1){
                btnDelete.visible = false;
            };
        }
        ...
        override public function notify(_arg1:Object):void{
			...
			if (_arg1.fData.cSel != null){
                iSel = _arg1.fData.cSel;
            };
            fDraw();
        }

		private function onChatShowTTOut(_arg_1:MouseEvent):void
        {
			var rootClass:* = MovieClip(stage.getChildAt(0));
			rootClass.ui.ToolTip.close();
        }
		
		private function onChatShowTTOver(_arg_1:MouseEvent):void
        {
			var rootClass:* = MovieClip(stage.getChildAt(0));
            rootClass.ui.ToolTip.openWith({str:"Show in Chat"});
        }
		
		private function onBtnChatShowClick(_arg_1:Event):void
        {
			var itemFilterName = iSel.sName;
			itemFilterName = itemFilterName.replace(rootClass.chatF.regExpLinking2, "$1")
			rootClass.chatF.mci.te.htmlText = rootClass.chatF.mci.te.htmlText + " <a href='loadItem:"+ iSel.CharItemID +"'>&lt;" + itemFilterName + "&gt;</a> ";	
			rootClass.chatF.mci.te.htmlText = rootClass.chatF.mci.te.htmlText.replace(rootClass.chatF.regExpSPACE, " ");
			rootClass.chatF.openMsgEntry();
        }
		
		public function chatShowRequest(_arg_1:Object):void
        {
            if (_arg_1.accept)
            {
                rootClass.world.sendChatShowItemRequest(iSel);
            };
        }
                            ...
                            if ((((_arg1.sType.toLowerCase() == "item")) && (!((String(_arg1.sLink).toLowerCase() == "none"))))){
                                ...
                                if (_arg1.sType.toLowerCase() == "egg"){
                                    ...
                                    if ((((_arg1.sType.toLowerCase() == "serveruse")) || ((_arg1.sType.toLowerCase() == "clientuse")))){
										...
										if ((((_arg1.sType.toLowerCase() == "resource")))){
											loadBag(_arg1);
										}
										else {
											clearPreview();
        ...
        private function onUpgradeTTOver(_arg1:MouseEvent):void{
            rootClass.ui.ToolTip.openWith({str:"This item is exclusive to upgraded VIP members."});
        }
